...
class SettingsTests(unittest.TestCase):
    ...
    def test_openai_strategy_settings_load_secret_from_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            key_path = Path(tmp_dir) / "strategy-openai-key"
            key_path.write_text("strategy-secret\n", encoding="utf-8")
            with patch.dict(
                "os.environ",
                {
                    "AI_RISK_STRATEGY_BACKEND": "openai",
                    "AI_RISK_STRATEGY_OPENAI_BASE_URL": "https://strategy.example.com/v1",
                    "AI_RISK_STRATEGY_OPENAI_MODEL": "gpt-5-mini",
                    "AI_RISK_STRATEGY_OPENAI_TIMEOUT_SEC": "13.0",
                    "AI_RISK_STRATEGY_OPENAI_REASONING_EFFORT": "medium",
                    "AI_RISK_STRATEGY_OPENAI_MAX_OUTPUT_TOKENS": "420",
                    "AI_RISK_STRATEGY_OPENAI_API_KEY_FILE": str(key_path),
                },
            ):
                config = AppConfig.from_env()

        self.assertEqual(config.strategy_backend, "openai")
        self.assertEqual(config.strategy_openai_base_url, "https://strategy.example.com/v1")
        self.assertEqual(config.strategy_openai_model, "gpt-5-mini")
        self.assertEqual(config.strategy_openai_timeout_sec, 13.0)
        self.assertEqual(config.strategy_openai_reasoning_effort, "medium")
